...
public:
  ...
  bool hasCoordinates();
  bool isLockedDownStream();
  bool isLockedUpStream();
  bool isReversedTraffic();
  void sendDataChangedSignal(int trackID);
  void setEndStation( int stationID);
  void setLockDownStream(bool lockDS);
  void setLockUpStream(bool lockUS);
