...
{ /* Verify that the train has no current station after creation */
  nc->addTrainToNetwork("testTrain");
  EXPECT_THAT(nc->trainList.at(0)->getNbrOfPassengers(),
              Eq(0));
  int deniedPassengers = nc->trainList.at(0)->load(3);
  EXPECT_THAT(nc->trainList.at(0)->getNbrOfPassengers(),
              Eq(3));
  EXPECT_THAT(deniedPassengers,
              Eq(0));
...
{ /* Verify that the train has no current station after creation */
  nc->addTrainToNetwork("testTrain");
  EXPECT_THAT(nc->trainList.at(0)->getCurrentStation(),
              Eq(UNDEFINED));
}
...
{ /* Verify that the train has no current track after creation */
  nc->addTrainToNetwork("testTrain");
  EXPECT_THAT(nc->trainList.at(0)->getCurrentTrack(),
              Eq(UNDEFINED));
}
...
{ /* Verify that the travel plan iS empty after creation */
  nc->addTrainToNetwork("testTrain");
  EXPECT_THAT(nc->trainList.at(0)->getTravelPlan().length(),
              Eq(0));
}
